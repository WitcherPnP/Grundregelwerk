...
Die Suche von häufig vorkommenden Ingredienzen ist einfach: Einmal pro Suche muss der Spieler eine Probe auf \textit{Pflanzenkunde} bestehen. Die Probe kann durch äußere Einflüsse erleichtert oder erschwert werden, wie bei einer Probe auf \textit{Sinnerschärfe} (Sicht). Danach hat der Spieler einmal pro Ort eine 5/6 Chance, die gesuchte Ingredienz zu finden. Findet der Spieler nichts, kann er alle 30min am selben Ort suchen, bis er etwas gefunden hat. D.h., dass an einem Ort nur einmal etwas gefunden werden kann. Bei jedem folgenden Versuch wird die Chance (aber nicht die Menge) um 1/6 erhöht (maximale Erhöhung=2/6). Um die gesuchte Ingredienz zu finden wirft er einen W6. Bei 1 bis 5 hat es der Spieler gefunden. Das Wurfergebnis gibt gleichzeitig die Menge an, die er gefunden hat. Mit der Menge muss nicht unbedingt die Ingredienz selbst gemeint sein. Nachdem die Menge bestimmt wurde kann es sein, dass die Ingredienz eine \textit{Pflanzenkunde}-Probe verlangt, die mit einer festgelegten QS bestanden werden muss. Misslingt die Probe wird die Menge für jeden Punkt den der Spieler daneben lag um 1 reduziert.

Wenn der Spieler beispielsweise nach einer Beere sucht macht er eine Probe auf \textit{Pflanzenkunde}. Er besteht die Probe und würfelt einen W6. Er würfelt eine 6. D.h., dass er in den ersten 30min nichts gefunden hat. Nach 30min besteht er eine erneute \textit{Pflanzenkunde}-Probe und würfelt erneut einen W6. Da die Chance diesmal auf 6/6 (5/6+1/6) gestiegen ist, findet er auf jeden Fall etwas. Er würfelt eine 6. Da zwar die Chance gestiegen ist, aber nicht die maximale Menge die er finden kann, zählt die 5. Laut SL bedeutet das, dass der Spieler 5 Sträucher gefunden hat, an dem die Beeren wachsen. Im Herbarium steht über die Beere, dass der Spieler für jeden Strauch einen W6 würfeln darf um die Menge der Beeren zu bestimmen\footnote{Das steht nicht wirklich im Herbarium, da es nur ein Beispiel ist}. Er würfelt eine 1, eine 3 und eine 4. Somit gibt es 8 Beeren die er pflücken kann. Da die Beeren sehr empfindlich sind verlangen sie eine Probe mit QS 2 (steht im Herbarium des Grundregelwerks zu dieser Beere). Der Spieler schafft die Probe leider nur mit 2 Punkten, also QS 1. Da er zwei FP unter QS 2 ist, bekommt er 8-2=6 Beeren. Jetzt sind 60min vorbei.
...
\subsection{Grundstoffe}
\label{chap:grundstoffe}
Tränke, Öle und Bomben benötigen bestimmte Grundstoffe zur Herstellung. 

\subsubsection{Fett}
\label{chap:fett}
Öle benötigen Fette, damit sie 24h an Gegenständen haften können. Leichte Öle benötigen \textit{normales Fett} (Qualität \textit{normal}), z.B. tierisches Fett, das für wenige Kronen von Metzgern oder Bauern erworben werden kann. Mittlere Öle benötigen \textit{hochwertiges Fett} (Qualität \textit{hoch}) und Schwere Öle \textit{qualitativ hochwertiges Fett} (Qualität \textit{top}).

\begin{longtable}{|l|l|l|}
\hline
\textbf{Fett} & \textbf{Qualität} & \textbf{Wert (Kr)} \\ \hline
\textbf{Alchemistische Paste} & top & 70 \\ \hline
\textbf{Bärenfett} & top & 40 \\ \hline
\textbf{Hundetalg} & top & 10 \\ \hline
\textbf{Gänseschmalz} & hoch & 20 \\ \hline
\textbf{Nierenfett} & hoch & 20 \\ \hline
\textbf{Andere Fette} & normal & 5 \\ \hline

\caption{Fette}
\label{tab:fette}
\end{longtable}

\subsubsection{Pulver}
Bomben benötigen Pulver als Grundstoff. Leichte und mittlere Bomben benötigen \textit{hochwertiges Pulver} (Qualität \textit{hoch}). Schwere Bomben benötigen \textit{qualitativ hochwertiges Pulver} (Qualität \textit{top}).

\begin{longtable}{|l|l|l|}
\hline
\textbf{Fett} & \textbf{Qualität} & \textbf{Wert (Kr)} \\ \hline
\textbf{Alchemisten Pulver} & top & 70 \\ \hline
\textbf{Serrikanische Mischung} & top & 40 \\ \hline
\textbf{Salpeter} & hoch & 20 \\ \hline
\textbf{Schwarzpulver\footnote{siehe Kapitel \ref{chap:schwarzpulver} Schwarzpulver}} & hoch & 35 \\ \hline
...
\subsubsection{Andere Flüssigkeiten}
